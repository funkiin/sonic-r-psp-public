...
static float s_drawQuadFarClip;
static float s_drawQuadInvFarClip;
                      ...
                      unsigned int color)
{
    ...
    if (farClip != s_drawQuadFarClip) {
        s_drawQuadFarClip = farClip;
        s_drawQuadInvFarClip = reciprocal(farClip);
    }
    float z = z_raw * s_drawQuadInvFarClip;
...
static RenderVertex BuildModelVertex(const SrcVertex *sv, int uvU16, int uvV16,
                                     float invFarClipSafe, int alpha)
{
    RenderVertex rv;
    int pz = sv->depth;
    SrcVertex_ProjectFloat(sv, &rv.sx, &rv.sy);
    rv.sz = (float)pz * invFarClipSafe;
...
//    R_SetFilter(R_FILTER_NEAREST);
    ...
    float invFarClipSafe = reciprocal(farClipSafe);
    RenderVertex rv[3];
    for (int i = 0; i < 3; i++) {
        rv[i] = BuildModelVertex(verts[i], poly[i * 2], poly[i * 2 + 1], invFarClipSafe, alpha);
    }
    R_DrawTri(rv);
#endif
}
                    ...
                    int rotation, int angleC, intptr_t objPtr, int scale)
{
    ...
    float invScale = (scale != 0) ? reciprocal((float)scale) : 1.0f;
    ...
    for (unsigned int vi = 0; vi < vertCount; vi++) {
        ...
        /* Scale if requested */
        if (scale != 0) {
            vx = vx * invScale; // / scale;
            vy = vy * invScale; // / scale;
            vz = vz * invScale; // / scale;
        }
