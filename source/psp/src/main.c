...
#include <pspdebug.h>
#include <pspkernel.h>
#define PSP_STARTUP_PRINTF(...) do { \
    pspDebugScreenInit(); \
    pspDebugScreenPrintf(__VA_ARGS__); \
} while (0)
#else
#define PSP_STARTUP_PRINTF(...) do { } while (0)
#endif

#ifdef SONICR_PSP
static void psp_startup_fatal(const char *fmt, ...)
{
    char buf[512];
    va_list args;
    pspDebugScreenInit();
    pspDebugScreenPrintf("Sonic R PSP startup stopped:\n\n");
    va_start(args, fmt);
    vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    pspDebugScreenPrintf("%s", buf);
    pspDebugScreenPrintf("\nFix the issue, then reset the game.\n");
    for (;;) {
        sceKernelDelayThread(100000);
    }
}

static const char *psp_data_dir_from_argv(int argc, char **argv)
{
    static char ebootDir[256];

    for (int i = 0; i < argc && argv != NULL; i++) {
        if (argv[i] == NULL || argv[i][0] == '\0') {
            continue;
        }
        if (strncmp(argv[i], "umd0:", 5) == 0 || strncmp(argv[i], "disc0:", 6) == 0) {
            continue;
        }
        if (strncmp(argv[i], "host0:", 6) == 0) {
            const char *path = argv[i] + 6;
            if (strstr(path, ".prx") != NULL || strstr(path, ".PRX") != NULL ||
                strstr(path, ".PBP") != NULL || strstr(path, ".pbp") != NULL) {
                return "host0:/";
            }
        }
        if (argv[i][0] == '/') {
        ...
        strncpy(ebootDir, argv[i], sizeof(ebootDir) - 1);
        ebootDir[sizeof(ebootDir) - 1] = '\0';

        char *slash = strrchr(ebootDir, '/');
        if (slash != NULL) {
            *slash = '\0';
            if (ebootDir[0] != '\0') {
                return ebootDir;
...
static int psp_arg_is_launcher_path(const char *arg)
{
    if (arg == NULL || arg[0] == '\0') {
        return 1;
    }
    return strncmp(arg, "umd0:", 5) == 0 ||
           strncmp(arg, "disc0:", 6) == 0 ||
           strstr(arg, ".prx") != NULL ||
           strstr(arg, ".PRX") != NULL ||
           strstr(arg, ".PBP") != NULL ||
           strstr(arg, ".pbp") != NULL;
...
int main(int argc, char *argv[])
{
    ...
    int explicitDataPath = (optind < argc);
#ifdef SONICR_PSP
    PSP_STARTUP_PRINTF("argc=%d\n", argc);
    for (int i = 0; i < argc; i++) {
        PSP_STARTUP_PRINTF("argv[%d]=%s\n", i, argv[i] ? argv[i] : "(null)");
    }
    if (explicitDataPath && psp_arg_is_launcher_path(argv[optind])) {
        PSP_STARTUP_PRINTF("Ignoring launcher path: %s\n", argv[optind]);
        explicitDataPath = 0;
    }
#endif
    if (explicitDataPath) {
    ...
    if (explicitDataPath) { /* explicit data path wins */
        if (chdir(dataDir) != 0) {
            fprintf(stderr, "Cannot chdir to data directory: %s\n", dataDir);
            PSP_STARTUP_PRINTF("Cannot chdir to data directory:\n%s\n", dataDir);
#ifdef SONICR_PSP
            psp_startup_fatal("Cannot chdir to data directory:\n%s\n", dataDir);
#endif
            return 1;
        }
    }
...
#ifdef SONICR_PSP
        const char *base = psp_data_dir_from_argv(argc, argv);
#else
    ...
    if (!probe) {
        ...
        PSP_STARTUP_PRINTF("Game data not found.\nPut EBOOT.PBP next to GENERAL/, BIN/, etc.\n");
#ifdef SONICR_PSP
        psp_startup_fatal("Game data not found in current directory.\n"
                          "Expected GENERAL/SONICR.BIT beside EBOOT.PBP.\n"
                          "Put GENERAL/, BIN/, AI/, SOUND/, etc. in the same folder.");
#endif
        return 1;
    }
    fclose(probe);
...
def SONICR_DC /* DC splash screen */
    ...
    if (net_transport_init() != 0) {
        ...
        PSP_STARTUP_PRINTF("Network disabled on PSP for now.\n");
#if !defined(__EMSCRIPTEN__) && !defined(SONICR_PSP)
        return 1;
#endif
    }
#ifdef SONICR_DC
