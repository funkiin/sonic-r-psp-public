...
#include <math.h>
#if defined(SONICR_SDL) || defined(SONICR_PSP)
#include "platform.h"
#endif
#include <sys/stat.h>
#include <errno.h>
...
extern const char *g_raceSubModeNames[4];
extern const char *g_trackDirNames[6];

void InitSaveData(void) {
    if (mkdir(PATH_SAVE, 0755) != 0 && errno != EEXIST) {
        DebugLog("InitSaveData: mkdir(%s) failed, errno=%d\n", PATH_SAVE, errno);
    }

    if (mkdir(PATH_GHOST, 0755) != 0 && errno != EEXIST) {
        DebugLog("InitSaveData: mkdir(%s) failed, errno=%d\n", PATH_GHOST, errno);
    }

    for (int subMode = 0; subMode < 4; subMode++) {
        char subDir[256];
        snprintf(subDir, sizeof(subDir), "%s/%s", PATH_GHOST, g_raceSubModeNames[subMode]);
        ...
        for (int track = 1; track <= 5; track++) {
            char trackDir[256];
            snprintf(trackDir, sizeof(trackDir), "%s/%s", subDir, g_trackDirNames[track]);
...
void LoadGameState(void)
{
    ...
    /* Far clip bounds — binary sets these at 0x4CD8xx after InitDefaultTimeTables.
